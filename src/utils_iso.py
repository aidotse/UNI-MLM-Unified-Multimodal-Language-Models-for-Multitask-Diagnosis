...
def train_epoch(projectors, optimizers, mse_loss, train_loader, device, llm, beta):
    ...
    Details:
        - The following is done for each projector in the list:
    ...
    gammas = [5,5,2,4,2,4,4,4,2,4,5,5]
    train_loss_batches = []
    ...
    for batch_index, (x, y) in enumerate(train_loader, 1):
        ...
        logits = [logit_extractor(encoded[i], llm) for i in range(len(encoded))]

        loss_asym = [custom_asym_loss(logits[i], labels, asym_fns) for i in range(len(logits))]
        loss_mse = custom_mse_loss(decoded, mse_inputs, mse_loss)

        losses = [loss1 + beta * loss2 for loss1, loss2 in zip(loss_asym, loss_mse)]
...
def validate(projectors, mse_loss, val_loader, device, llm, beta):
    """
    ...
    with torch.no_grad():
        for batch_index, (x, y) in enumerate(val_loader, 1):
            ...
            loss_asym = custom_asym_loss(logits, labels, asym_fns)
            loss_mse = custom_mse_loss(decoded, mse_inputs, mse_loss)
...
def training_loop(projectors, optimizers, mse_loss, train_loader, val_loader, num_epochs, llm, beta):
    ...
    for epoch in range(1,num_epochs+1):
        projectors, train_loss = train_epoch(projectors, optimizers, mse_loss, train_loader, device, llm, beta)

        val_loss = validate(projectors, mse_loss, val_loader, device, llm, beta)
