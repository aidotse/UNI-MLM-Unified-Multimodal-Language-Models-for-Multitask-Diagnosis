...
for y in transposed_Y:
    ...
    num_zeros = (y == 0).sum().float()
    num_ones = (y == 1).sum().float()
    
    # Calculate weights as the inverse of class frequencies
    weight_zero = n / (2 * K * num_zeros + 1e-6) 
    weight_one = n / (2 * K * num_ones + 1e-6)    

    weights = torch.tensor([weight_zero, weight_one], dtype=torch.float).to("cuda")
    weight_per_class.append(weights)
...
for weight in weight_per_class:
    loss_fns.append(nn.BCEWithLogitsLoss(weight=weight))

num_epochs = 50
beta = 0.1

#Run training
training_loop(projectors, optimizers, mse_loss, loss_fns, train_loader, val_loader, num_epochs, gemma, beta)
