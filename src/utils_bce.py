...
def custom_bce_loss(logits, labels, loss_fns):
    """
    Computes a multi-label BCE loss using specified loss_fns.
    ...
    Returns:
        loss (float): Average BCE loss across all classes.
    ...
    Details:
        ...
        - Aggregates the losses and computes the average loss.
    """
    ...
    for i, fn in enumerate(loss_fns):
        ...
        if masked_labels.size(0) == 0:
            ...
            loss = fn(masked_logits, masked_labels) 
            losses.append(loss)
...
def train_epoch(projectors, optimizers, mse_loss, loss_fns, train_loader, device, llm, beta):
    """
    Trains the projectors for one epoch using MSE and BCE loss functions.

    Parameters:
        projectors (list): List of projector models.
        optimizers (list): List of optimizers for each projector.
        mse_loss (function): Mean Squared Error loss function.
        loss_fns (list): List of BCE loss functions for each class.
    ...
    for batch_index, (x, y) in enumerate(train_loader, 1):
        ...
        loss_bce = custom_bce_loss(logits, labels, loss_fns)
        loss_mse = custom_mse_loss(decoded, mse_inputs, mse_loss)

        losses = [loss_bce + beta * loss for loss in loss_mse]
        ...
        loss = sum([loss.item() for loss in losses]) - 5*loss_bce.item()
        train_loss_batches.append(loss)

    return projectors, train_loss_batches




def validate(projectors, mse_loss, loss_fns, val_loader, device, llm, beta):
    """
...
def training_loop(projectors, optimizers, mse_loss, loss_fns, train_loader, val_loader, num_epochs, llm, beta):
    """
    ...
    for epoch in range(1,num_epochs+1):
        projectors, train_loss = train_epoch(projectors, optimizers, mse_loss, loss_fns, train_loader, device, llm, beta)

        val_loss = validate(projectors, mse_loss, loss_fns, val_loader, device, llm, beta)
